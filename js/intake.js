...
(() => {
  ...
  // ========================================
  const CONFIGS = {
    'Live Production Consulting': {
      variant: 'sage',
      eyebrow: 'Live Production',
      headline: "Book<br/>the board.",
      sub: "Tell me about the show. I'll tell you what it takes to make it land.",
      submitText: 'Book the Board',
      fields: [
        ...
        { name: 'show_name', label: 'Event or show', type: 'text', required: true, placeholder: 'Sunday service, tour, worship night…' },
        { name: 'event_date', label: 'Event date (optional)', type: 'text', required: false, placeholder: 'Month/year or "ongoing"' },
        { name: 'role', label: 'What role are you?', type: 'select', required: true, options: ['Artist or band', 'Church', 'Corporate event', 'Venue or production co.', 'Other'] },
        { name: 'scope', label: 'What do you need?', type: 'textarea', required: true, placeholder: 'Audio, lighting, livestream, show flow — tell me what\'s broken or what you\'re building.' },
      ],
    },
    default: {
      variant: 'lime',
      ...
      fields: [
        ...
        { name: 'email', label: 'Email', type: 'email', required: true, placeholder: 'you@company.com' },
        { name: 'phone', label: 'Phone (optional)', type: 'tel', required: false, placeholder: '555-555-5555' },
        { name: 'notes', label: 'What are you trying to make happen?', type: 'textarea', required: true, placeholder: 'Tell us what you need built.' },
      ],
    },
  };
...
#intake-modal.variant-sage { --intake-accent: #6a9686; --intake-accent-text: #131318; --intake-accent-glow: rgba(106, 150, 134, 0.4); }
...
<div id="intake-modal" class="fixed inset-0 z-[100] opacity-0 invisible pointer-events-none transition-all duration-300 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="intake-title">
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const open = (service) => {
      ...
      // Swap variant class for accent color
      modal.classList.remove('variant-lime', 'variant-sage');
      modal.classList.add(`variant-${config.variant}`);
